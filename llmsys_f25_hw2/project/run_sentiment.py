...
# Set custom download path for embeddings
# Change this to your desired path
# Note: embeddings library will create a subdirectory structure like: glove/wikipedia_gigaword.zip
EMBEDDINGS_CACHE_DIR = "/mnt/fsx/fsx-pred/prediction/tdvi"
os.environ['EMBEDDINGS_ROOT'] = EMBEDDINGS_CACHE_DIR
os.makedirs(EMBEDDINGS_CACHE_DIR, exist_ok=True)

# Add parent directory to path for minitorch import
# This allows the script to find minitorch module
...
# Choose backend: SimpleOps (CPU) or CudaKernelOps (GPU)
# from minitorch import SimpleOps
# BACKEND = minitorch.TensorBackend(SimpleOps)

# To use CUDA backend, uncomment the following lines:
...
def cross_entropy_loss(out, y):
    ...
    # Binary Cross Entropy: -[y * log(out) + (1-y) * log(1-out)]
    # ones is needed for tensor arithmetic (to compute 1-y and 1-out)
    ones = minitorch.tensor_functions.ones(y.shape, backend=BACKEND)
    
    # Direct BCE calculation (NOT using logsoftmax - that's for multi-class!)
    return -(y * out.log() + (ones - y) * (ones - out).log()).mean()
...
class Linear(minitorch.Module):
    ...
    def forward(self, x):
        ...
        weights_view = self.weights.value.view(in_size, self.out_size)
        
        # Matrix multiplication: (batch, in_size) @ (in_size, out_size) -> (batch, out_size)
        out = x_view @ weights_view
        out = out.view(batch, self.out_size)
        
        # Add bias
        bias_view = self.bias.value.view(1, self.out_size)
        out = out + bias_view
...
class Network(minitorch.Module):
    ...
    def forward(self, embeddings):
        ...
        # Step 1: Average over sentence length dimension (dim=1)
        # embeddings shape: (batch, sentence_length, embedding_dim)
        averaged = embeddings.mean(dim=1)  # (batch, 1, embedding_dim) or (batch, embedding_dim)
        
        # Ensure it's 2D by viewing to (batch, embedding_dim)
        batch_size = embeddings.shape[0]
        embedding_dim = embeddings.shape[2]
        averaged = averaged.view(batch_size, embedding_dim)
        
        # Step 2: Apply first linear layer
        hidden = self.linear1.forward(averaged)  # (batch, hidden_dim)
        
        # Step 3: Apply ReLU and dropout
        hidden = hidden.relu()
        hidden = minitorch.dropout(hidden, self.dropout_prob, ignore=not self.training)
        
        # Step 4: Apply second linear layer
        logits = self.linear2.forward(hidden)  # (batch, 1)
        
        # Step 5: Apply sigmoid and reshape
        output = logits.sigmoid()
        output = output.view(output.shape[0])  # (batch,)
    ...
    def train(
        ...
        for epoch in range(1, max_epochs + 1):
            ...
            if data_val is not None:
                ...
                for val_example_num in range(0, n_validation_samples, batch_size):
                    # Handle the last batch which might be smaller
                    end_idx = min(val_example_num + batch_size, n_validation_samples)
                    if val_example_num >= end_idx:
                        break
                    
                    x = minitorch.tensor(X_val[val_example_num:end_idx], backend=BACKEND)
                    y = minitorch.tensor(y_val[val_example_num:end_idx], backend=BACKEND)
