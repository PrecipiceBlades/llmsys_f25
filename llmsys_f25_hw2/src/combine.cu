...
__global__ void mapKernel(
   ...
   */
    ...
    int in_index_pos = index_to_position(in_index, in_strides, shape_size);
    int out_index_pos = index_to_position(out_index, out_strides, shape_size);
    out[out_index_pos] = fn(fn_id, in_storage[in_index_pos]);
}
...
__global__ void zipKernel(
   ...
   */
    ...
    int a_index_pos = index_to_position(a_index, a_strides, a_shape_size);
    int b_index_pos = index_to_position(b_index, b_strides, b_shape_size);
    int out_index_pos = index_to_position(out_index, out_strides, out_shape_size);
    out[out_index_pos] = fn(fn_id, a_storage[a_index_pos], b_storage[b_index_pos]);
    /// END ASSIGN2_2
}
...
__global__ void reduceKernel(
   ...
   */
    ...
    // to_index(out_pos, out_shape, out_index, shape_size);
    // int out_index_pos = index_to_position(out_index, out_strides, shape_size);
    // out[out_index_pos] = reduce_value;
    // for (int i = 0; i < a_shape[reduce_dim]; i++) {
    //     out_index[reduce_dim] = i;
    //     int a_index_pos = index_to_position(out_index, a_strides, shape_size);
    //     reduce_value = fn(fn_id, reduce_value, a_storage[a_index_pos]);
    // }
    // out[out_index_pos] = reduce_value;
    ...
    // 优化的并行归约实现
    // ============================================================
    // 使用 shared memory 和树形归约来加速
    // 每个 block 处理一个输出元素的归约
    __shared__ float cache[BLOCK_DIM];
    int tid = threadIdx.x;  // 线程在 block 内的位置
    int out_pos = blockIdx.x;  // 每个 block 负责一个输出元素
    // ============================================================
    // 步骤 1: 确定输出位置和初始化
    // ============================================================
    if (out_pos >= out_size) {
    ...
    to_index(out_pos, out_shape, out_index, shape_size);
    // ============================================================
    // 步骤 2: 每个线程加载并归约一部分数据到 shared memory
    // ============================================================
    // 每个线程负责处理 reduce_dim 上的多个元素
    float thread_reduce_val = reduce_value;
    ...
    __syncthreads();
    // ============================================================
    // 步骤 3: 树形归约（在 shared memory 中）
    // ============================================================
    // 使用二叉树归约模式：步长从 1 开始，每次翻倍
    // 例如: [a,b,c,d,e,f,g,h]
    // s=1: [a+b, c+d, e+f, g+h]
    // s=2: [a+b+c+d, e+f+g+h]
    // s=4: [a+b+c+d+e+f+g+h]
    for (int s = 1; s < blockDim.x; s *= 2) {
        ...
        __syncthreads();  // 确保所有线程完成当前层的计算
    }
    // ============================================================
    // 步骤 4: Thread 0 将最终结果写回全局内存
    // ============================================================
    if (tid == 0) {
        int out_index_pos = index_to_position(out_index, out_strides, shape_size);
        out[out_index_pos] = cache[0];
    }
    /// END ASSIGN2_3
}
...
__global__ void MatrixMultiplyKernel(
   ...
   */
    ...
    // ============================================================
    // 步骤 1: 计算当前线程负责的输出元素位置
    // ============================================================
    // 每个 block 计算输出矩阵的一个 TILE×TILE 块
    // 每个线程负责该块中的一个元素
    int row = blockIdx.y * TILE + threadIdx.y;  // 输出矩阵的行
    int col = blockIdx.x * TILE + threadIdx.x;  // 输出矩阵的列
    
    // 矩阵维度
    int m = a_shape[1];  // A 的行数
    int n = a_shape[2];  // A 的列数 = B 的行数
    int p = b_shape[2];  // B 的列数
    ...
    for (int tile_idx = 0; tile_idx < num_tiles; tile_idx++) {
        // 当前 tile 在 K 维度的起始位置
        int tile_k = tile_idx * TILE;
        
        // ========================================================
        // 步骤 3a: 加载 A 的一个 tile 到 shared memory
        // ========================================================
        // A[row, tile_k : tile_k+TILE]
        int a_col = tile_k + threadIdx.x;
        if (row < m && a_col < n) {
            // 计算在 batch 中的实际位置
            int a_idx = batch * a_batch_stride + row * a_strides[1] + a_col * a_strides[2];
            a_shared[threadIdx.y][threadIdx.x] = a_storage[a_idx];
        } else {
            a_shared[threadIdx.y][threadIdx.x] = 0.0;
        }
        
        // ========================================================
        // 步骤 3b: 加载 B 的一个 tile 到 shared memory
        // ========================================================
        // 每个线程加载 B[tileK + ty, blockCol*TILE + tx]
        int b_row = tile_k + threadIdx.y;
        int b_col = blockIdx.x * TILE + threadIdx.x;  // 使用 block 的列索引
        if (b_row < n && b_col < p) {
            int b_idx = batch * b_batch_stride + b_row * b_strides[1] + b_col * b_strides[2];
            b_shared[threadIdx.y][threadIdx.x] = b_storage[b_idx];
        } else {
            b_shared[threadIdx.y][threadIdx.x] = 0.0;
        }
        
        // ========================================================
        // 步骤 4: 同步，确保 tile 数据加载完成
        // ========================================================
        __syncthreads();
        
        // ========================================================
        // 步骤 5: 计算当前 tile 的部分矩阵乘法
        // ========================================================
        // 计算当前 tile 的有效 K 范围
        int tile_k_end = min(TILE, n - tile_k);
        for (int k = 0; k < tile_k_end; k++) {
            sum += a_shared[threadIdx.y][k] * b_shared[k][threadIdx.x];
        }
        // ========================================================
        // 步骤 6: 同步，确保所有线程完成当前 tile 的计算
        // ========================================================
        // 在下一次迭代前，确保没有线程还在使用 shared memory
        __syncthreads();
    }
    ...
    // ============================================================
    if (row < m && col < p) {
        int out_idx = batch * out_strides[0] + row * out_strides[1] + col * out_strides[2];
        out[out_idx] = sum;
    }
...
void MatrixMultiply(
    ...
    // gridDims: (列数/TILE, 行数/TILE, batch)
    // 因为 kernel 中 blockIdx.x 对应列，blockIdx.y 对应行
    dim3 gridDims((p + threadsPerBlock - 1) / threadsPerBlock, 
...
void tensorReduce(
    ...
    // Launch kernel
    // 优化版本：每个 block 处理一个输出元素
    // 使用多个线程协作完成单个输出的归约
    int threadsPerBlock = BLOCK_DIM;  // 使用 BLOCK_DIM (1024) 个线程
    int blocksPerGrid = out_size;      // 每个输出元素一个 block
